...
class Execute {
  ...
  public:
    ...
    Execute(Register *r, predict *pre) : regist(r), p(pre) {}
    ...
    void perform() {
        ...
        case JALR: {
            ...
            // B-Type
        case BEQ:
            instruction.res = static_cast<uint>(instruction.src1 == instruction.src2);
            break;
        case BNE:
            instruction.res = static_cast<uint>(instruction.src1 != instruction.src2);
            break;
        ...
        case BGEU:
            ...
            break;
        case BLT:
            instruction.res = static_cast<uint>((int)instruction.src1 < (int)instruction.src2);
            break;
        case BGE:
            instruction.res = static_cast<uint>((int)instruction.src1 >= (int)instruction.src2);
            break;
        ...
        case ADDI:
            ...
            break;
        case SLTI:
            instruction.res = static_cast<uint>((int)instruction.src1 < (int)instruction.imm);
            break;
        case SLTIU:
            instruction.res = static_cast<uint>(instruction.src1 < instruction.imm);
            break;
        case ANDI:
            instruction.res = (instruction.src1 & instruction.imm);
            break;
    ...
    bool judge() {
        if (instruction.type == BEQ || instruction.type == BNE || instruction.type == BLT || instruction.type == BGE ||
