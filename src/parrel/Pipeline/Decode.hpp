...
class Decode {
	...
	public:
        ...
        //加入分支预测
        void branchpredict(){
            if(instruction.type==BEQ||instruction.type==BNE||instruction.type==BGE||instruction.type==BLT||instruction.type==BGEU||instruction.type==BLTU){
                instruction.pre=pre->prediction(instruction.type);
                //若预测成功，在ID阶段就修改PC
                if(instruction.pre) regist->write_pc(instruction.resultpc-4+instruction.imm);
            }
         }
	void back(){
